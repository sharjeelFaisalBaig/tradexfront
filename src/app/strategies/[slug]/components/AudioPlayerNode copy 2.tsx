...
export default function AudioUploadNode({
        ...
        { strategyId, peerId: data?.id, data: formData },
        {
          onSuccess: (response) => {
            ...
            analyzeAudioContent(
              ...
              {
                onSuccess: () => {
                  setIsPollingRestarting(true); // Set to true before restarting
                  restartPolling();
                  setProcessingState({
                    isProcessing: false,
                    isComplete: true,
  ...
  }, [currentError, processAudioFile, analyzeAudioContent, strategyId, data]);
  ...
  const handleRemoveAudio = useCallback(() => {
    ...
    resetPeer(
      { peerId: data?.id, strategyId, peerType: "audio" },
      {
        ...
        onSuccess: () => {
          setIsPollingRestarting(false); // Ensure polling is stopped
        },
      }
    );
