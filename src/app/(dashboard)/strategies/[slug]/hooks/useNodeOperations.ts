...
export const useNodeOperations = () => {
  ...
  const { mutate: createChartPeer } = useCreateChartPeer();
  ...
  const { mutate: deleteChartPeer } = useDeleteChartPeer();
    ...
    ({
      ...
      switch (tool) {
        case "annotation":
          ...
          break;
        case "group":
          // For group nodes, we can add them directly without API call since they're UI-only containers
          setNodes((nodes) => [
            ...nodes,
            {
              ...newNode,
              id: crypto.randomUUID(), // Generate a unique ID for group nodes
              data: {
                ...(Object.keys(dataToAutoUpload)?.length > 0
                  ? { dataToAutoUpload }
                  : {}),
  ...
  const deleteNode = useCallback(
    (nodeId: string, nodeType: string, strategyId: string) => {
      ...
      const deleteMutationMap: Record<string, any> = {
        ...
        chartNode: deleteChartPeer,
        groupContainer: null, // Group containers don't need API deletion
      };
      ...
      } else if (nodeType === "groupContainer") {
        // Group containers are UI-only, just remove from state
        removeNodeFromState();
      } else {
        toast({
          title: "Error",
  ...
  const updateNodeSize = useCallback(
    (nodeId: string, width: number, height: number, minWidth = 300) => {
