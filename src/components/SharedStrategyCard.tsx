...
function getImageUrl(img?: string) {
  if (!img) return "https://images.unsplash.com/photo-1590283603385-17ffb3a7f29f?w=400&h=200&fit=crop";
  if (img.startsWith("http")) return img;
  return DOMAIN_ROOT + img;
}
...
interface SharedStrategyCardProps {
  strategy: any;
  invitationMeta: any;
  isStarred: boolean;
  onStarToggle: () => void;
...
export default function SharedStrategyCard({
  strategy,
  invitationMeta,
  isStarred,
  onStarToggle,
  onCopy,
}: SharedStrategyCardProps) {
  return (
    <Card className="group hover:shadow-lg transition-shadow cursor-pointer overflow-hidden rounded-[10px]">
      <CardContent className="p-0">
        ...
        <div className="aspect-video px-5 mb-4">
          <Image
            src={getImageUrl(strategy.image)}
            alt={strategy.name}
            width={400}
            height={200}
        ...
        <div className="px-5 pb-4">
          <div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">
            <span>
              Shared At: {formatDate(invitationMeta.invited_at)}<br />
              Status: {invitationMeta.action}<br />
              Invited By:{" "}
              {invitationMeta.invited_by?.avatar && (
                <Image
                  src={getImageUrl(invitationMeta.invited_by.avatar)}
                  alt={invitationMeta.invited_by.name}
                  width={20}
                  height={20}
                  className="inline-block rounded-full mr-1"
                />
              )}
              {invitationMeta.invited_by?.name}
            </span>
            <div className="flex items-center space-x-2">
              ...
              <Clipboard className="h-6 w-5 cursor-pointer" onClick={onCopy} />
            </div>
          </div>
        </div>
      </CardContent>
