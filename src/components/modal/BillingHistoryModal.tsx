...
export default function BillingHistoryModal({
  ...
  const [filterOptions, setFilterOptions] = useState<any>({ purposes: [], statuses: [] })
  ...
  const fetchBillingHistory = async () => {
    ...
    try {
      ...
      const data = await fetchWithAutoRefresh(`${endpoints.BILLING.HISTORY}?${params.toString()}`, session)
      if (data?.status) {
  ...
  const fetchFilterOptions = async () => {
    try {
      const data = await fetchWithAutoRefresh(endpoints.BILLING.FILTER_OPTIONS, session)
      if (data?.status) {
        ...
        <Dialog.Panel className="bg-white dark:bg-background rounded-lg shadow-lg p-6 w-full max-w-6xl">
          ...
          <div className="flex gap-4 mb-4">
            <Select onValueChange={(value) => handleFilterChange('purpose', value)}>
              ...
              <SelectContent>
                {filterOptions.purposes.map((opt: any) => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}
              </SelectContent>
            </Select>
            <Select onValueChange={(value) => handleFilterChange('status', value)}>
              <SelectTrigger><SelectValue placeholder="Status" /></SelectTrigger>
              <SelectContent>
                {filterOptions.statuses.map((opt: any) => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}
              </SelectContent>
            </Select>
            <DatePicker
                ...
                <tbody>
                  {billingHistory.map((entry) => (
                    <tr key={entry.id} className="border-t">
                      ...
                      <td className="p-2 border">{new Date(entry.processed_at).toLocaleDateString()}</td>
          ...
          <div className="flex justify-between items-center mt-4">
            <Select onValueChange={(value) => setFilters(prev => ({ ...prev, per_page: Number(value), page: 1 }))} defaultValue={String(filters.per_page)}>
            ...
            <div className="flex gap-2">
              <Button onClick={() => setFilters(prev => ({ ...prev, page: prev.page - 1 }))} disabled={pagination.current_page === 1}>Prev</Button>
              <span>Page {pagination.current_page} of {pagination.last_page}</span>
              <Button onClick={() => setFilters(prev => ({ ...prev, page: prev.page + 1 }))} disabled={pagination.current_page === pagination.last_page}>Next</Button>
            </div>
          </div>
        </Dialog.Panel>
      </div>
