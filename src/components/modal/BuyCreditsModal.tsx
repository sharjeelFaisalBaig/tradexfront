...
function BuyCreditsForm({ onSuccess, onClose }: { onSuccess: (data: any) => void, onClose: () => void }) {
  ...
  useEffect(() => {
    async function fetchCreditInfo() {
      try {
        const data = await fetchWithAutoRefresh(endpoints.CREDITS.INFO, session)
        if (data?.status) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!confirmResponse?.status) {
        throw new Error(confirmResponse?.message || 'Failed to confirm credits purchase.')
      }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="mb-4">
        ...
        />
        {creditInfo && (
          <p className="text-sm text-muted-foreground mt-2">{creditInfo.pricing_note}</p>
        )}
      </div>
      <div className="mb-4">
        <label className="block text-sm font-medium text-muted-foreground dark:text-gray-400 mb-2">
      ...
      <Button type="submit" disabled={!stripe || loading || !creditInfo?.can_purchase_credits} className="w-full h-12 text-lg">
        {loading ? 'Processing...' : `Pay $${(credits * (creditInfo?.per_credit_price || 0)).toFixed(2)}`}
      </Button>
    </form>
  )
}
        ...
        <Dialog.Panel className="relative bg-white dark:bg-background rounded-lg shadow-lg p-8 w-full max-w-md">
          ...
          {successData ? (
            <div className="text-center">
              ...
              <p><strong>Credits Purchased:</strong> {successData.credits_purchased}</p>
              <p><strong>New Credit Balance:</strong> {successData.new_credit_balance}</p>
              <Button onClick={handleClose} className="mt-6">Close</Button>
            </div>
          ) : (
            <>
              <Dialog.Title className="text-2xl font-bold text-center mb-6">Buy Credits</Dialog.Title>
              <Elements stripe={stripePromise}>
                <BuyCreditsForm onSuccess={handleSuccess} onClose={handleClose} />
              </Elements>
            </>
          )}
        </Dialog.Panel>
